...
// var test1 = Tribonacci(new double[] { 0, 0, 1 }, 10);
var test2 = Tribonacci(new double[] { 11, 17, 12 }, 2);
...
double[] Tribonacci(double[] signature, int n)
{
    ...
    if (n <= 3)
        return seq.Where((e, i) => i < n).ToArray();
    ...
    {
        double temp = 0;
        for (int i = seq.Count - 1; i > (seq.Count - 1) - 3 ; i--)
        {
            temp += seq[i];
        }
        seq.Add(temp);
